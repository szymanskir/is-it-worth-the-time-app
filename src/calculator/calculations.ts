...
function shiftToLargestUnit(duration: Duration): [number, TimeUnit] {
    const units: TimeUnit[] = Object.values(TimeUnit);

    let result = duration.as(TimeUnit.Second);
    let resultUnit = TimeUnit.Second

    for (let i = 0; i < units.length; i++) {
        const shifted = duration.as(units[i])
...
export function calculatePotentialSavedTime(calculationInput: TimeCalculationInput): [number, TimeUnit] {
    ...
    const savedTime = Math.trunc(taskDurationInResultUnits * taskOccurenceCount);

    return shiftToLargestUnit(formDurationToLuxonDuration(savedTime, calculationInput.resultUnit));
}
